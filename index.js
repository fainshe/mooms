...
function title() {
      console.clear()
      console.log(chalk.yellow(`\n\n               ${chalk.bold.yellow(`Mooms Bot`)}\n\n`))
      console.log(color(`< ================================================== >`, 'cyan'))
	console.log(color(`\n${themeemoji} Don don for a Don don!`,'magenta'))
console.log(color(`${themeemoji} GITHUB: fainshe `,'magenta'))
console.log(color(`${themeemoji} Stay moomin for a better life and less of error.`,'magenta'))
...
console.log(color(figlet.textSync(`MoomsBot`, {
font: 'Standard',
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
			...
			if (res.announce == true) {
				...
				XeonBotInc.sendMessage(res.id, {
					text: `「 Setelan Grup 」\n\nGrup ditutup oleh admin !`,
				});
			} else if (res.announce == false) {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Setelan Grup 」\n\nGrup dibuka oleh admin !`,
				});
				...
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, { 
					text: `「 Informasi Grup 」\n\n*Deskripsi grup telah diganti menjadi*\n\n${res.desc}`,
				});
      } else {
				await sleep(2000)
				XeonBotInc.sendMessage(res.id, {
					text: `「 Informasi Grup」\n\n*Nama grup diganti menjadi*\n\n*${res.subject}*`,
				});
			} 
...
│「 𝗚𝗼𝗼𝗱𝗯𝘆𝗲 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
   │✑  Selamat menjadi nomor yang abu abu..
   │✑ Keluar pada:  ${xeontime} ${xeondate}
   └───────────────┈ ⳹`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `  @${xeonName.split("@")[0]}, diangkat jadi *admin* `
   XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `@${xeonName.split("@")[0]}, di  *demoted* dari*admin* 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
if (reason === DisconnectReason.badSession) { console.log(`File sesi bad/kadaluarsa, Hapus sesi dan scan ulang.`); XeonBotInc.logout(); }
else if (reason === DisconnectReason.connectionClosed) { console.log("Koneksi tutup, reconnecting...."); XeonBotIncBot(); }
else if (reason === DisconnectReason.connectionLost) { console.log("Koneksi hilang dari server, reconnecting..."); XeonBotIncBot(); }
else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Sesi lainnya terkoneksi, mohon tutup sesi ini terlebih dahulu" ); XeonBotInc.logout(); }
else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, mohon Scan ulang..`); XeonBotInc.logout(); }
else if (reason === DisconnectReason.restartRequired) { console.log("Restart dibutuhkan. Merestart..."); XeonBotIncBot(); }
else if (reason === DisconnectReason.timedOut) { console.log("Koneksi timeout, mungkin dikarenakan sinyal yang buruk. Reconnecting..."); XeonBotIncBot(); }
else XeonBotInc.end(`Unknown DisconnectReason: ${reason}|${connection}`)
} else if (connection === "open") { XeonBotInc.sendMessage(owner + "@s.whatsapp.net", { text: `[ INFO ] Bot started, untuk me-recode bisa langsung ke update tanpa harus restart bot. Terimakasih` }); }
